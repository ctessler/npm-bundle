...
Evaluation of \tpj{} and the non-preemptive multi-threaded task model
presented in this work focuses on the schedulability ratio of
synthetic task sets. The ratio of task set specifications deemed
schedulable by \tpj{} for EDF-NP will be compared to \npchunks{} in
both limited and fully preemptive settings for EDF. What follows is a 
description of the parameters to task set specification generation,
...
% \input{\tex{10-specification-parameters.tex}}

\subsubsection{Synthetic Tasks and Applicability of Parameters}

Synthetic task sets are the basis for the evaluation out of
necessity. Currently, we are unaware of any hard real-time
multi-threaded task sets that have been accepted as representative of
real-world applications suitable for research. While such a task set
would be a benefit, it would also likely require similar extrapolation
techniques to match the more than 500,000 task sets considered herein.
...
cache benefits and analysis of \texttt{BUNDLE} scheduling. EDF-P:M
performs analysis assuming preemptions between jobs are allowed and
...
for the configuration. Call ${s}$ the set of ${\tasks{}^1}$
transformed from ${\ants{} \in S}$ such that ${\tasks{}^1}$ are
infeasible for EDF-P:1 due to the utilization of ${\tasks{}^1}$
exceeding one. The set \stpj{} is the subset of ${s}$ deemed feasible
by the \tpj{} schedulability test. That is, \stpj{} is the set of all
tasks \tpj{} could schedule, but EDF-P:1 could not -- even considering
EDF-P:1 does not account for CRPD overhead that
EDF-P:1 would incur in a real system implementation. 
